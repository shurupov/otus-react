...
describe("RightOperationProcessor extractOperation method", () => {
  ...
  it("Find Operation 10**", () => {
    ...
    expect(found).toEqual(true);
    expect(result).toEqual({
      operation: operations.SQUARE_INLINE,
      arguments: ["10"],
    });
  });
...
describe("RightOperationProcessor performOperation method", () => {
  it("Find 6!", () => {
    expect(processor.performOperation(operations.FACTORIAL, [6])).toEqual(
      6 * 5 * 4 * 3 * 2
    );
  });
  it("Find 6**", () => {
    expect(processor.performOperation(operations.SQUARE_INLINE, [6])).toEqual(
      36
    );
  });
});
