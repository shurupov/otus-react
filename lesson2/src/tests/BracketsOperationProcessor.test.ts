...
describe("BracketsOperationProcessor extractOperation method", () => {
  ...
  test.each([
    ...
    expect(
      processor.isOperationExtracted(operation + "(" + argument + ")", result)
    ).toEqual(true);
...
describe("BracketsOperationProcessor performOperation method", () => {
  it("sin(30)", () => {
    expect(processor.performOperation(operations.SIN, [30])).toEqual(
      Math.sin((30 * Math.PI) / 180)
    );
  });
  it("tg(30)", () => {
    expect(processor.performOperation(operations.TG, [30])).toEqual(
      Math.tan((30 * Math.PI) / 180)
    );
  });
  it("ctg(30)", () => {
    expect(processor.performOperation(operations.CTG, [30])).toEqual(
      1 / Math.tan((30 * Math.PI) / 180)
    );
  });
  it("cos(30)", () => {
    expect(processor.performOperation(operations.COS, [30])).toEqual(
