...
export class ReversePolishNotationCalculator {
  ...
  protected availableOperations: string[] = this.oneArgumentOperations.concat(
    this.twoArgumentsOperations
  );
  ...
  public calc(expressionContainer: ExpressionContainer): number {
    ...
    for (const availableOperation of this.availableOperations) {
      if (
        expression.substr(
          position - availableOperation.length + 1,
          availableOperation.length
        ) === availableOperation
      ) {
        operation = availableOperation;
        //console.log("Found operation: " + operation);
        expressionContainer.expression = expression.substr(
          0,
          position - operation.length + 1
        );
    ...
    do {
      ...
      startPosition > 0 &&
      expression.substr(startPosition - 1, 1) != " "
    );
  ...
  public performOperation(
    ...
    if (this.twoArgumentsOperations.includes(operation)) {
      ...
      result = this.performTwoArgumentsOperation(
        operation,
        argument1,
        argument2
      );
  ...
  public performTwoArgumentsOperation(
    operation: string,
    argument1: number,
    argument2: number
  ): number {
  ...
  public performOneArgumentOperation(
    operation: string,
    argument: number
  ): number {
    switch (operation) {
      case operations.FACTORIAL:
        return this.factorial(argument);
      case operations.SQUARE_INLINE:
        return argument * argument;
      case operations.SIN:
        return Math.sin(this.degreesToRadians(argument));
      case operations.COS:
        return Math.cos(this.degreesToRadians(argument));
      case operations.TG:
        return Math.tan(this.degreesToRadians(argument));
      case operations.CTG:
        return 1 / Math.tan(this.degreesToRadians(argument));
      case operations.SQR:
        return argument * argument;
